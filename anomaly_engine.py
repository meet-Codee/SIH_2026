...
CONTAMINATION      = 0.06 # Isolation Forest expected anomaly fraction (≈ 6%)
...
def run_pipeline(sensor_df: pd.DataFrame, nbrs: dict) -> tuple[pd.DataFrame, IsolationForest]:
    ...
    # Keep a backward-compatible "confidence" alias for the app.py
    ...
    # Hybrid correction: if the deterministic rule engine found a known fault
    # or a genuine event, ensure 'is_anomaly' flag matches the outcome.
    # This prevents IF false-negatives on subtle anomalies like drift.
    df["is_anomaly"] = df["root_cause"] != RC_NORMAL
...
def _estimate_corrected_values(df: pd.DataFrame, nbrs: dict) -> pd.DataFrame:
    """
    Estimate the true underlying value for anomalous readings.

    Strategy (in priority order):
      1. Normal readings → corrected = raw (unchanged, no modification)
      2. Anomalous reading, normal neighbours available
         → corrected = mean of normal neighbours at same timestamp
      3. Anomalous reading, all neighbours also anomalous
         → corrected = station's rolling temporal mean (recent non-anomalous history)
      4. Insufficient data → corrected = raw (flagged with uncertainty)

    The corrected values are ESTIMATES. Raw readings are always preserved.
    """
    df = df.sort_values(["station_id", "timestamp"]).copy()

    # Pre-compute rolling temporal means (for fallback) — computed on raw values
    for col in ["temp", "humidity", "pressure"]:
        df[f"_{col}_roll_mean"] = df.groupby("station_id")[col].transform(
            lambda x: x.rolling(ROLLING_WINDOW, min_periods=2).mean().bfill().ffill()
        )
    ...
    hum_lut      = df_indexed["humidity"]
    pres_lut     = df_indexed["pressure"]
    anom_lut     = df_indexed["is_anomaly"]

    corrected_t, corrected_h, corrected_p = [], [], []

    for _, row in df.iterrows():
        if not row["is_anomaly"]:
            corrected_t.append(row["temp"])
            corrected_h.append(row["humidity"])
            corrected_p.append(row["pressure"])
            continue

        # Gather values from non-anomalous neighbours
        nbr_t, nbr_h, nbr_p = [], [], []
        for nbr in nbrs.get(row["station_id"], []):
            key = (row["timestamp"], nbr)
            if key in anom_lut.index and not anom_lut[key]:
                nbr_t.append(temp_lut[key])
                nbr_h.append(hum_lut[key])
                nbr_p.append(pres_lut[key])

        if nbr_t:
            corrected_t.append(round(float(np.mean(nbr_t)), 2))
            corrected_h.append(round(float(np.mean(nbr_h)), 2))
            corrected_p.append(round(float(np.mean(nbr_p)), 2))
        else:
            # Fallback: station's own rolling mean
            corrected_t.append(round(float(row["_temp_roll_mean"]),     2))
            corrected_h.append(round(float(row["_humidity_roll_mean"]), 2))
            corrected_p.append(round(float(row["_pressure_roll_mean"]), 2))

    df["temp_corrected"]     = corrected_t
    df["humidity_corrected"] = corrected_h
    df["pressure_corrected"] = corrected_p

    # Remove temporary columns
    df.drop(columns=["_temp_roll_mean", "_humidity_roll_mean", "_pressure_roll_mean"],
