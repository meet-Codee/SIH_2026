...
def fetch_historical_observations(stations: list[dict], hours: int = 72) -> Optional[pd.DataFrame]:
    ...
    all_records = []
    # Using a simple loop instead of async/ThreadPool to avoid complex dependencies 
    # and keep it strictly synchronous and stable.
    for st in stations:
        features = _fetch_station_data(st, hours=hours)
        all_records.extend(_parse_features(features, st))
