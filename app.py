...
if MODE == "simulated":
    ...
    def _live_data_refresher():
        ...
        Runs automatically every 90 s. Only triggers a full-page rerun
        when the data actually changes, preventing unnecessary blinking.
        """
        ...
        last_refresh = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        # First load or explicit refresh
        need_fetch_frag = (
        ...
        if need_fetch_frag or do_snapshot_frag:
            with st.spinner("Fetching live observations from Open-Meteo…"):
                if do_snapshot_frag:
            ...
            if live_full is None or live_full.empty:
                st.session_state.live_error = (
                    ...
                    "Try a different range or use Snapshot Only."
                )
                st.rerun()  # surface error on main page
                return
            ...
            # Record previous timestamp to detect genuine data change
            prev_ts = (
            ...
            if prev_ts != new_ts or need_fetch_frag:
                # Trigger ONE full-page rerun so all tabs reflect new data.
                # Between data changes the fragment auto-runs silently
    ...
    if st.session_state.live_df is None:
        st.info("Click **Refresh Now** above to load live observations.")
        st.stop()

    full_df   = st.session_state.live_df
...
if view_mode == "Anomalies Only":
    snapshot_display = snapshot[snapshot["is_anomaly"]]
...
# ---------------------------------------------------------------------------
badge_html = {
    "simulated": '<span class="badge-sim">● SIMULATED</span>',
    "live":      '<span class="badge-live">● LIVE IMD/AWS</span>',
    "csv":       '<span class="badge-csv">● HISTORICAL CSV</span>',
}[MODE]
...
if MODE == "live":
    st.info(
        ...
        f"{full_df['station_id'].nunique()} stations | "
        f"{len(all_timestamps)} hourly snapshots | "
...
anomalous_now = int(snapshot["is_anomaly"].sum())
genuine_now   = int((snapshot["root_cause"] == "Genuine Weather Event (not a fault)").sum())
faults_now    = anomalous_now - genuine_now
critical_now  = int((snapshot.get("severity", pd.Series(dtype=str)) == "CRITICAL").sum())
...
with tabs[0]:
    ...
    with map_col:
        ...
        for _, row in snapshot_display.iterrows():
            ...
            popup = (
                f"<div style='width:220px;font-family:sans-serif;font-size:12px'>"
                f"<b>{row['name']}</b><br>"
                f"Root cause: <b style='color:{color}'>{row['root_cause']}</b><br>"
                f"Anomaly score: {conf:.0f}% &nbsp;|&nbsp; Confidence: {cc:.0f}%<br>"
                f"Severity: <b>{sev}</b>"
                f"<hr style='margin:4px 0'>"
                f"Temp: {row['temp']:.1f}&deg;C (est: {row['temp_corrected']:.1f})<br>"
            ...
            folium.CircleMarker(
                ...
                tooltip=f"{row['name']} | {row['root_cause']} | Sev: {sev}",
            ).add_to(m)
    ...
    with log_col:
        ...
        st.dataframe(
            snapshot_display[[c for c in disp if c in snapshot_display.columns]]
            .sort_values("anomaly_score_pct", ascending=False),
...
with tabs[1]:
    ...
    station_name = st.selectbox("Select station", sorted(full_df["name"].unique()))
    sub = full_df[
        (full_df["name"] == station_name) & (full_df["timestamp"] <= current_time)
    ]
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines", name="Raw reading", line=dict(color="#3498db", width=1.5),
    ))
    if f"{param}_corrected" in sub.columns:
        fig.add_trace(go.Scatter(
            x=sub["timestamp"], y=sub[f"{param}_corrected"],
            mode="lines", name="AI-corrected estimate",
            line=dict(color="#2ecc71", dash="dot", width=1.5),
        ))
    anom_sub = sub[sub["is_anomaly"]]
    if not anom_sub.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                ...
                size=[14 if s in ("CRITICAL", "HIGH") else 10 for s in sev_col],
                symbol="x",
            ),
            text=[
                f"{rc} | Sev: {sv} | Score: {sc:.0f}%"
                for rc, sv, sc in zip(
                    anom_sub["root_cause"],
    ...
    fig.update_layout(
        height=430, yaxis_title=label, xaxis_title="Time",
    ...
    st.caption(
        "Dotted green: AI-corrected estimate (neighbour spatial mean; falls back to "
        "station rolling mean). Marker size reflects severity."
    )
    if not sub.empty:
        with st.expander("Engineered features — latest observation"):
            latest = sub.iloc[-1]
            feat_data = {f: round(float(latest[f]), 4) for f in FEATURE_COLS if f in latest}
            st.dataframe(
                pd.DataFrame(list(feat_data.items()), columns=["Feature", "Value"]),
                use_container_width=True, hide_index=True,
            )
...
with tabs[2]:
    ...
    if not flagged_idx:
        ...
        flagged = full_df.loc[flagged_idx].sort_values("anomaly_score_pct", ascending=False)
        choice = st.selectbox(
            "Select event",
            flagged.index,
...
with tabs[4]:
    ...
    log = window_df[window_df["is_anomaly"]].sort_values("timestamp", ascending=False).copy()
    action_map = {
    ...
    if HAS_GT:
        ...
        if "error" in eval_results:
            ...
            cfg = {
                ...
                "Contamination": f"{CONTAMINATION:.1%}", "Features": len(FEATURE_COLS),
            ...
            st.dataframe(
                pd.DataFrame(list(cfg.items()), columns=["Parameter", "Value"]),
                use_container_width=True, hide_index=True,
            )
        ...
        st.markdown("---")
        st.markdown(f"**Time range:** `{ts_min}` → `{ts_max}` ({hrs:.1f} h)")

        if MODE == "live":
            st.markdown("---")
            st.markdown("### Data Provenance & Transparency")
            st.markdown("**SOURCE:** Open-Meteo API")
            st.markdown("**DATA TYPE:** ERA5/ECMWF Reanalysis & Forecast (ingests WMO/SYNOP surface observations)")
            st.markdown(f"**LAST FETCH:** {ts_max} UTC")
            st.markdown(f"**STATIONS:** {n_stations}")
            st.markdown(f"**OBSERVATIONS:** {total_obs}")
            st.markdown("**FIELDS:** Temperature | Relative Humidity | Surface Pressure")
            st.markdown("**PIPELINE:** Same SkyGuard AI anomaly-detection pipeline")
            st.caption("Note: Official IMD direct API requires registration/whitelisting. Open-Meteo provides verified proxy data. Not direct IMD AWS.")
